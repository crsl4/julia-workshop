...
- In the benchmark each language implementation requires code to parse the .bed files. In this case, they are simple tab-separated-value files.
...
- [Tables.jl](https://github.com/JuliaData/Tables.jl) provides an interface between row-oriented tables, such as that produced by `CSV.File` and column-oriented tables, such as stored by `Arrow.write`.
...
```{julia}
function chromooverlap(targetdict, refdict, nm)
  refs = refdict[nm]
  targets = targetdict[nm]
