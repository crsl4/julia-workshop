...
- This code chunk downloads the tarball, if necessary, and extracts the two bedfiles to a directory `biofast-data-v1`, if necessary.
...
- Versions of Julia prior to 1.8.0-rc2 will give incorrect results on the coverage.
:::
...
- If we were not allowing for multiple threads it would be more idiomatic to write the loop as
```julia
result = sizehint!(typeof(coverage(first(targets), refs))[], length(targets))
for t in targets
  push!(result, coverage(t, refs))
end
```
```{julia}
