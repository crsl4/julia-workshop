...
- [This repository](https://github.com/lh3/biofast) contains yet another benchmark for comparing programming languages - in this case on two "typical" Bioinformatics tasks.
...
- Parsing a CSV or TSV file can be tedious and error prone.
...
- We can see that the table is row-oriented but we can save it to a Arrow, column-oriented file directly because both the `Arrow` and `CSV` packages implement the `Tables` interface.
...
function coverage(needle::UnitRange{T}, haystack::Vector{UnitRange{T}}) where {T}
  s = filter(!isempty, intersect.(Ref(needle), haystack))
  start = first(needle) - oneunit(T)
  stop = last(needle)
  nover = T(length(s))
  noverbp = T(length(mapreduce(BitSet, union, s; init=BitSet())))
...
function chromooverlap(needledict, haystackdict, nm)
  hs = haystackdict[nm]
  nvec = needledict[nm]
  return DataFrame([coverage(f, hs) for f in nvec])
end
```

- this method works but is slow because of memory allocations and garbage collection
...
- much of the memory allocation can be eliminated if a vector for the results of the intersection is passed to `coverage`

```{julia}
function coverage!(
  intsect::Vector{UnitRange{T}},
  s::Vector{UnitRange{T}},
  needle::UnitRange{T},
  haystack::Vector{UnitRange{T}}
) where {T}
  empty!(s)    # resets length to 0 but does not release storage
  map!(h -> intersect(needle, h), intsect, haystack)
  for isect in intsect
    isempty(isect) || push!(s, isect)
  end
  start = first(needle) - oneunit(T)
  stop = last(needle)
  nover = T(length(s))
...
```{julia}
let 
  needle = rnadict["chr5"][2]
  haystack = annodict["chr5"]
  intsect = similar(haystack)
  s = similar(haystack)
  coverage!(intsect, s, needle, haystack)
end
```

- Redefine `chromooverlap` to use `coverage!` instead of `coverage`.

```{julia}
function chromover(needledict, haystackdict, nm)
  hs = haystackdict[nm]
  nvec = needledict[nm]
  ivec = similar(hs)
  s = similar(hs)
  return DataFrame([coverage!(ivec, s, f, hs) for f in nvec],)
end
```

```{julia}
chr21tbl = chromover(rnadict, annodict, "chr21")
```

- But the performance is not very good

```{julia}
@time chromover(rnadict, annodict, "chr21");
```