...
- Modify the approach to use [IntervalTrees.jl](https://github.com/BioJulia/IntervalTrees.jl) and multi-threading.
...
- [Li Heng](https://liheng.org) provides benchmark code and sample data for comparing programming languages on Bioinformatics tasks in the [biofast](https://github.com/lh3/biofast) repository.
...
::: {.callout-note}
The call `joinpath.(datadir, ["ex-anno.bed", "ex-rna.bed"])` is an example of [dot broadcasting](https://docs.julialang.org/en/v1/manual/functions/#man-vectorized)
:::
...
- We use a `replace!` method takes the object to modify and one or more `pair`s of values of the form `from => to` and generate the replacements as
...
```
::: {.callout-note}
Representing the `chromo` column as a `CategoricalArray` converts the individual values to indices into a vector of `String`s, which saves on space.
:::
...
- A general framework for working with tabular data is provided in [Tables.jl](https://github.com/JuliaData/Tables.jl) (not shown here).
- We have shown the use of
