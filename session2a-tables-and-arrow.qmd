...
- The `replace!` generic takes the object to modify and one or more `pair`s of values of the form `from => to`
...
- In the next section we will use data from `chr21` for comparative timings, because there are a small, but nontrivial, number of intervals in both the reference and target groups.
