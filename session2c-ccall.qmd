...
annotbl = rowtable(DataFrame(Arrow.Table("./biofast-data-v1/ex-anno.arrow")));
rnatbl = rowtable(DataFrame(Arrow.Table("./biofast-data-v1/ex-rna.arrow")));
```

## Background

- [Li Heng](https://liheng.org) wrote the [biofast](https://github.com/lh3/biofast) benchmarks to compare implementations of his [cgranges](https://github.com/lh3/cgranges) interval tree method.
...
```{julia}
libcgranges = "./lib/libcgranges.so"
...
```{julia}
function cr_add(crptr::Ptr{cgranges_t}, r::NamedTuple, i::Integer)
  cr_add(crptr, r.chromo, r.start, r.stop, i)
end
```

- The `cr_add` function takes an extra integer argument that is stored as a label.
```{julia}
...
m_b_ = Ref(0)
r = first(rnatbl)
noverlap = cr_overlap(crptr, r.chromo, r.start, r.stop, b_, m_b_)
```

- There are 12 overlapping reference intervals for this first target interval
...
```{julia}
(; start=cr_start(crptr, 7), stop=cr_end(crptr, 7), label=cr_label(crptr, 7))
```
...
```{julia}
nover = [cr_overlap(crptr, r.chromo, r.start, r.stop, b_, m_b_) for r in rnatbl]
```

- This is very fast (remember that this timing is for all the chromosomes).
```{julia}
@time [cr_overlap(crptr, r.chromo, r.start, r.stop, b_, m_b_) for r in rnatbl];
```
