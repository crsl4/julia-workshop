...
```{julia}
@code_warntype chromodict(rowtable(Arrow.Table("biofast-data-v1/ex-anno.arrow")))
```
...
function overlaps(targets::Vector{UnitRange{T}}, refs::RangeNode{T}) where {T}
  ...
  result = similar(targets, ceil(Int, sqrt(treesize(refs))))
  for tar in targets
