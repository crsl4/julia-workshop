...
- Create two utilities, `datatable` to read the Arrow file and convert it to a `DataFrame`, and `chromodict` to convert the DataFrame to a `Dict{Symbol,Vector{UnitRange}}`, optionally first sorting the DataFrame.
...
rangetree21 = annorangetrees[:chr21]  # tree represented by its root node
```

::: {.callout-note}

Each node in the tree contains `ranges`, the `Vector{UnitRange}` from which the tree was created, and `maxlast`, a `Vector{<:Integer}` of the maximum `last` values of the ranges in the subtree rooted at each node, and `inds`, the index range encompassed by the node.  (The `ranges` and `maxlast` fields are pointers, there is no copying of vectors when moving around the tree.)

```{julia}
propertynames(rangetree21)
```
...
```{julia}
treesize(rangetree21), treeheight(rangetree21), treebreadth(rangetree21)  # number of leaf nodes
```

```{julia}
print_tree(rangetree21; maxdepth = 4)
```
...
:::
  ...
  - One check on the calculation is that the `maxlast` value for the root node must be the maximum last position of any of the ranges.

```{julia}
maximum(last.(rangevec21))
```

  - It happens that this value is also the last position in the last range but that doesn't have to be the case.  The ranges are sorted by increasing first position, not by last position.
...
function coveragecount(
  ...
  target = eltype(isects)(target)  # coerce the type of target
  goalposts = target
