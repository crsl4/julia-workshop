...
:::
  ...
  - Create a utility, `chromodict` to take an `Arrow.Table` and convert it to a `Dict{Symbol, Vector{UnitRange{T}}}`, assuming that the table contains columns named "chromo", "start", and "stop".

```{julia}
function chromodict(tbl::Arrow.Table)
  vtype = Vector{UnitRange{Tables.columntype(tbl, :start)}}
...
tarrngvecs = chromodict(Arrow.Table(joinpath(datadir, "ex-rna.arrow")))
refrngvecs = chromodict(Arrow.Table(joinpath(datadir, "ex-anno.arrow")))
```
...
The expression `(; chromo, start, stop) = r` is equivalent to three local assignments
```julia
chromo = r.chromo
start = r.start
stop = r.stop
...
[Tables.jl](https://github.com/JuliaData/Tables.jl) provides interfaces for row- and column-oriented tables, allowing for one orientation to be viewed as the other.  In this case an `Arrow.Table` is column-oriented but the `Tables.rows` method returns a low-overhead iterator over the rows.
:::
...
function overlaps(targets::Vector{UnitRange{T}}, refs::RangeNode{T}) where {T}
  ...
  result = similar(targets, Int(round(sqrt(length(refs.maxlast)))))
  for tar in targets
