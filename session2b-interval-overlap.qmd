...
:::
  ...
  - In `refrngvecs`, each of the values, which is a `Vector{UnitRange}`, should be sorted by the `first` element of the `UnitRange`.  For conversion to `IntervalTree`s below they are also required to be sorted by `last` if the `first` elements are equal.
...
savedresult = intersect(target, refrngvec21)  # will use for comparisons
```

```{julia}
result = similar(refrngvec21, 0)    # working storage
savedresult == intersect!(result, target, refrngvec21)
```
...
@benchmark intersect!(res, tar, ref) setup =
  (res = result; tar = target; ref = refrngvec21)
```

```{julia}
@benchmark intersect(tar, ref) setup = (tar = target; ref = refrngvec21)
```
...
function overlaps(targets::Vector{UnitRange{T}}, refs::RangeNode{T}) where {T}
  ...
  nover = sizehint!((treesize(refs) ≤ typemax(Int32) ? Int32 : Int64)[], ntargets)
...
overlaps(tarrngvec21, rangetree21)
```

```{julia}
@benchmark overlaps(targets, refs) setup =
  (targets = tarrngvec21; refs = rangetree21)
```
